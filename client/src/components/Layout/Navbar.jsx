  ...
  return (
    <nav className="sticky top-0 z-40 glass-nav transition-all duration-200">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          <div className="flex items-center space-x-3">
            ...
            {isAuthenticated ? (
              <div className="relative">
                <button
                  ...
                  className="flex items-center gap-2 p-1.5 pr-2.5 rounded-xl border border-slate-200/80 bg-white/70 hover:bg-white shadow-sm transition-all focus:outline-none"
                >
                  <img
                ...
                {dropdownOpen && (
                  <>
                    ...
                    />
                    <div className="absolute right-0 mt-2 w-52 bg-white/95 backdrop-blur-lg rounded-2xl shadow-xl border border-slate-200/90 py-2 z-20 animate-in fade-in duration-150">
              ...
              <div className="flex items-center space-x-2">
                ...
                <Link
                  to="/register"
                  className="px-3.5 py-1.5 text-xs font-semibold text-white bg-indigo-600 hover:bg-indigo-700 rounded-xl shadow-md transition-all"
                >
                  Get Started
                </Link>
              </div>
            ...
            <button
              ...
              className="p-2 rounded-xl text-slate-600 hover:bg-slate-100 md:hidden focus:outline-none"
        ...
        {mobileMenuOpen && (
          <div className="md:hidden border-t border-slate-200/80 pt-2 pb-4 px-2 space-y-1 animate-in fade-in slide-in-from-top-2 duration-150">
            {isAuthenticated && (
              <Link
                to="/dashboard"
                onClick={() => setMobileMenuOpen(false)}
                className={`flex items-center gap-2.5 px-4 py-2.5 rounded-xl text-sm font-semibold ${
                  isActive('/dashboard') ? 'bg-indigo-50 text-indigo-600' : 'text-slate-700 hover:bg-slate-100'
                }`}
              >
            ...
            <Link
              ...
              onClick={() => setMobileMenuOpen(false)}
              className={`flex items-center gap-2.5 px-4 py-2.5 rounded-xl text-sm font-semibold ${
                isActive('/problems') ? 'bg-indigo-50 text-indigo-600' : 'text-slate-700 hover:bg-slate-100'
              }`}
            >
              <BookOpen className="w-4 h-4" />
              Problems
            ...
            <Link
              ...
              onClick={() => setMobileMenuOpen(false)}
              className={`flex items-center gap-2.5 px-4 py-2.5 rounded-xl text-sm font-semibold ${
                isActive('/leaderboard') ? 'bg-indigo-50 text-indigo-600' : 'text-slate-700 hover:bg-slate-100'
              }`}
            >
              <Trophy className="w-4 h-4" />
              Leaderboard
            ...
            {isAdmin && (
              <Link
                ...
                onClick={() => setMobileMenuOpen(false)}
                className={`flex items-center gap-2.5 px-4 py-2.5 rounded-xl text-sm font-semibold ${
                  isActive('/admin') ? 'bg-purple-50 text-purple-700' : 'text-purple-600 hover:bg-purple-50'
                }`}
              >
                <ShieldAlert className="w-4 h-4" />
                Admin Console
            ...
            {isAuthenticated && (
              <button
                onClick={handleLogout}
                className="w-full flex items-center gap-2.5 px-4 py-2.5 rounded-xl text-sm font-semibold text-rose-600 hover:bg-rose-50 text-left"
              >
                <LogOut className="w-4 h-4" />
                Sign Out
              </button>
